...
import { SeriesCard } from '../components/ContentCard';
...
export default function SeriesScreen() {
  ...
  useEffect(() => {
    Promise.all([
      streamService.series(),
      categoryService.list('series')
    ]).then(([data, cats]) => {
      const list = data.series || [];
      setSeries(list);
      setHeroSeries(list[Math.floor(Math.random() * Math.min(5, list.length))]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <FlatList
        ...
        renderItem={({ item }) => (
          <SeriesCard item={item} onPress={handlePress} width={CARD_WIDTH} />
        )}
        ListHeaderComponent={
          <>
            {heroSeries && !search && !activeCategory && (
              <HeroBanner
                item={{ ...heroSeries, streamIcon: heroSeries.cover }}
                onPlay={handlePlay}
                onInfo={handlePress}
              />
            )}
